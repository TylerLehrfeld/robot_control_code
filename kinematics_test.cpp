...
void test_get_joint() {
    Point p = get_joint(true, LOWER_MIDPOINT_DISTANCE, LOWER_TRANSMISSION_LENGTH,
        LOWER_PROXIMAL_LENGTH, {0, 10, 0}, {-5, 5, 0}, inverse_robot.bottom_linkage);
    assert(is_close(p.x,  -8.08498));
    assert(is_close(p.y, 15.88498));
    assert(is_close(p.z, 0.0));
    Point p2 = get_joint(false, 6, 9, 12, {0, 10, 0}, {2, 4, 0}, inverse_robot.bottom_linkage);
...
void test_get_linkage_end_effector() {
    ...
    Point p = get_linkage_end_effector(true, {-5,5,0}, {5,6,0},{0,10,0}, UPPER_TRANSMISSION_LENGTH, UPPER_PROXIMAL_LENGTH, UPPER_DISTAL_LENGTH, UPPER_MIDPOINT_DISTANCE, {0,.5,0}, 0, inverse_robot);
}

int main() {
    //test_get_joint();
    //test_get_linkage_end_effector();
    Point end_effector = get_end_effector({-63, 109.08, 0}, {-21,144.91 ,0}, {21.07, 150.71, 0}, {63, 109.08, 0}, {0, 186.4, 0}, {0, 223.62, 0}, 0, inverse_robot);
