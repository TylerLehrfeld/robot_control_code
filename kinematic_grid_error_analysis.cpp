...
int main() {
  ...
  while (grid_file >> next_word) {
    ...
    Robot forward_robot;
    get_end_effector(sliders, forward_robot);
    Point ee = forward_robot.bottom_linkage.linkage_end_effector;
    double max_error = 0;
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        for (int k = -1; k <= 1; k++) {
          for (int I = -1; I <= 1; I++) {
            double slider_error = .35;
            slider_positions new_pos = {sliders.left_slider_y + slider_error * static_cast<double>(i),
                                        sliders.left_middle_slider_y + slider_error * static_cast<double>(j),
                                        sliders.right_middle_slider_y + slider_error * static_cast<double>(k),
                                        sliders.right_slider_y + slider_error * static_cast<double>(I),
                                        sliders.needle_extension};
            get_end_effector(new_pos, forward_robot);
            Point ee_adjusted = forward_robot.bottom_linkage.linkage_end_effector;
