...
slider_positions inverse_kinematics(
  ...
  Point upper_linkage_end_effector = get_linkage_end_effector(
    ...
    Point lower_linkage_end_effector = get_linkage_end_effector(needle_to_patient_approach, LOWER_LINKAGE_Z, false);
 ...
 * This essentially solves for y of a circle of radius transmission and center midpoint as a function of x.
 * 
 * @param midpoint 
 ...
 * @return float 
 */
float get_y_of_slider_based_on_midpoint_location(Point midpoint, float x, float transmission_length) {
    assert(midpoint.z == 0);
    return -1 * sqrt(pow(transmission_length, 2)-pow(x-midpoint.x,2)) + midpoint.y;
}


/**
 * @brief Get the slider positions for a linkage based on the end effector position.
 * 
...
void get_slider_positions(slider_positions& slider_positions_struct, Point end_effector, bool upper) {
  ...
  Point right_joint = intersection_of_two_circles(end_effector, upper ? UPPER_BASE : LOWER_BASE,
    upper ? UPPER_DISTAL_LENGTH : LOWER_DISTAL_LENGTH,
    upper ? UPPER_PROXIMAL_LENGTH : LOWER_PROXIMAL_LENGTH, false);
  Point base = upper ? UPPER_BASE : LOWER_BASE;
  float midpoint_distance = upper ? UPPER_MIDPOINT_DISTANCE : LOWER_MIDPOINT_DISTANCE;
  Point left_midpoint = ((left_joint - base).normalize() * LOWER_MIDPOINT_DISTANCE) + base;
  Point right_midpoint = ((right_joint - base).normalize() * LOWER_MIDPOINT_DISTANCE) + base;
  if(upper) {
    slider_positions_struct.left_slider_y = get_y_of_slider_based_on_midpoint_location(left_midpoint, sliderXs[0], UPPER_TRANSMISSION_LENGTH);
    slider_positions_struct.right_slider_y = get_y_of_slider_based_on_midpoint_location(right_midpoint, sliderXs[3], UPPER_TRANSMISSION_LENGTH); 
  } else {
    slider_positions_struct.left_middle_slider_y = get_y_of_slider_based_on_midpoint_location(left_midpoint, sliderXs[1], LOWER_TRANSMISSION_LENGTH);
    slider_positions_struct.right_middle_slider_y = get_y_of_slider_based_on_midpoint_location(right_midpoint, sliderXs[2], LOWER_TRANSMISSION_LENGTH);
  }
}