...
#ifndef GALIL_CALLS
#define GALIL_CALLS
const std::string galil_dir = "/home/tyler/Downloads/OneDrive_2025-07-19/Galil (originals)";
...
void init_galil(int home_reho_or_updown) {
    ...
    if (home_reho_or_updown == 1)
        check(GProgramDownloadFile(g, (galil_dir + "main.dmc").c_str(),
                                   "--max 4"));
    if (home_reho_or_updown == 2)
        check(
            GProgramDownloadFile(g, (galil_dir + "rc.dmc").c_str(), "--max 4"));
    if (home_reho_or_updown == 3) {
        check(GProgramDownloadFile(g, (galil_dir + "test.dmc").c_str(),
                                   "--max 4"));
    ...
    if (home_reho_or_updown == 4) {
        check(GProgramDownloadFile(g, (galil_dir + "spinA.dmc").c_str(),
                                   "--max 4"));
    }
...
int GoToBothBlocking(slider_positions sliders) {
    ...
    int valueL = 0;
    GCommand(g,
             ("tgtMmB = " + to_string(sliders.right_middle_slider_y)).c_str(),
             buf, G_SMALL_BUFFER, &read_bytes);
    GCommand(g, ("tgtMmE = " + to_string(sliders.left_middle_slider_y)).c_str(),
             buf, G_SMALL_BUFFER, &read_bytes);
...
void move_robot_with_slider_positions(slider_positions positions) {
    ...
    GSize read_bytes = 0;
    double right =
        BASE_TO_SLIDER_MAX - positions.right_slider_y - HALF_SLIDER_WIDTH;
    double left =
        BASE_TO_SLIDER_MAX - positions.left_slider_y - HALF_SLIDER_WIDTH;
    double right_middle = BASE_TO_SLIDER_MAX - positions.right_middle_slider_y -
                          HALF_SLIDER_WIDTH;
    double left_middle =
        BASE_TO_SLIDER_MAX - positions.left_middle_slider_y - HALF_SLIDER_WIDTH;
