...
int main() {
    ...
    Matrix rot(vector<Matrix>({forward_robot.x_prime.to_matrix(), forward_robot.y_prime.to_matrix(), forward_robot.z_prime.to_matrix()}));
    Transform expected_F_RN(rot, forward_robot.bottom_linkage.extended_end_effector.to_matrix());
    ...
    while(true) {
        ...
        //std::cout << "magnitude diff: " << (last_point - new_pos).magnitude() << std::endl;
        if(num_trials != 0)
            mag_tot += (last_point - new_pos).magnitude();
        num_trials++;
        last_point = new_pos;*/ 
